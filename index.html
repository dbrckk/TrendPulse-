...
(function () {
  ...
  async function renderBestSellerGrid() {
    ...
    const items = sortByScore(products.filter((p) => p.type === "catalog" && p.is_best_seller)).slice(0, 8);
    renderGrid("#best-seller-grid", items);
  }

  async function renderCheapTechGrid() {
    const el = document.getElementById("cheap-tech-grid");
    if (!el) return;

    const products = await fetchProducts();
    const items = sortByScore(
      products.filter((p) => p.type === "catalog" && p.category === "tech" && p.price <= 50)
    ).slice(0, 8);

    renderGrid("#cheap-tech-grid", items);
  }

  async function renderGiftGrid() {
    const el = document.getElementById("best-gifts-grid");
    if (!el) return;

    const products = await fetchProducts();
    const items = sortByScore(
      products.filter((p) => p.type === "catalog" && (p.is_giftable || p.category === "gifts"))
    ).slice(0, 8);

    renderGrid("#best-gifts-grid", items);
  }
  ...
  async function renderHomeCategoryGrid() {
    ...
    const products = await fetchProducts();
    const items = sortByScore(products.filter((p) => p.type === "catalog" && p.category === "home")).slice(0, 8);
    renderGrid("#home-grid", items);
  }
  ...
  async function renderKitchenGrid() {
    ...
    const products = await fetchProducts();
    const items = sortByScore(products.filter((p) => p.type === "catalog" && p.category === "kitchen")).slice(0, 8);
    renderGrid("#kitchen-grid", items);
  }
  ...
  async function renderDealsPage() {
    ...
    async function run() {
      ...
      let items = products.filter((p) => p.type === "deal");
